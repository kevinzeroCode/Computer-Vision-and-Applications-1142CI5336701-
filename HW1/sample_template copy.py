...
def load_xyz(path):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            points.append(np.array([X, Y, Z, 1.0]))
...
def load_camera_params(path):
    ...
    for i in range(NUM_FRAMES):
        ...
        K  = np.array(frame_data["intrinsic"],  dtype=np.float64)   # shape (3, 3)
        Rt = np.array(frame_data["extrinsic"],  dtype=np.float64)   # shape (3, 4)
...
def draw_marker(img, u, v, radius=18, color=(0, 0, 255), thickness=3):
    ...
    cv2.line(img, (u - radius, v), (u + radius, v), color, thickness)   # 橫線
    cv2.line(img, (u, v - radius), (u, v + radius), color, thickness)   # 直線
